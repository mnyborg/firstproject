...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "firstproject" because\n';
  ...
  $wnd.__gwt_activeModules['firstproject'] = {
 ...
 * "/firstproject.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['firstproject'].superdevmode = true;

  var expectedSuffix = '/firstproject.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + firstproject + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:firstproject';
  var devModeSessionKey = '__gwtDevModeSession:firstproject';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/firstproject/firstproject.nocache.js';
  ...
  $wnd.__gwt_activeModules['firstproject'].canRedirect = true;
  $wnd.__gwt_activeModules['firstproject'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING firstproject';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/firstproject?user.agent=' + ua + '&_callback=' + callback;
